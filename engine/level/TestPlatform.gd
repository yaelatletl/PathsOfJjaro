...
# TODO: we don't need platforms for Arrival Demo; however it'd be useful to get the behavior and visual structure worked out now; note: this current implementation is experimental and will require changes to make it fully reusable and configurable (see exported map JSONs for platform config dictionary)



# TODO: should doors/platforms be RigidBody3D or AnimatableBody3D? rigid is physics-dependent (which might be handy if we want them to respond "organically" to mass impacts, e.g. jumping onto a moving platform transfers some impulse to it, making it slightly bounce underfoot) whereas [animated] static body is code-controlled and needs its own Area collision detection to prevent pushing Player/NPCs through floor/wall/ceiling
#
...
# TODO: while we could use AnimationPlayer, it's probably best to stick to numeric values as these can be imported directly from the exported level JSONs

@export var active  := true : get = get_active, set = set_active # public; start/stop platform; TODO: not sure about naming as "active" could be confused with "enabled"
...
func __update_velocity() -> void:
	if moving:
		...
		__velocity = (v if move_up else -v) # TODO: may want to apply impulse (if RigidBody3D)
	else:
...
func _physics_process(delta: float):
	if moving:
		...
		if (remaining_distance > 0.01) if move_up else (remaining_distance < -0.01): # TODO: may want to reduce velocity when distance <10cm for more realistic platform acceleration/deceleration or soft-opening/closing door
			move_up = not move_up
			if wait_time > 0:
				__set_moving(false)
				__update_velocity()
