...
# Radar.gd -- confusingly, this draws health bars, not radar; this and CircularContainer.gd and CircularMeter.gd
...
func _ready():
	actor.connect("health_changed",Callable(self,"_on_health_changed")) # do we need signals for this? it could be convenient once global Inventory holds all persistent player stats, as it aavoids Inventory having to know about per-level Player objects; we can decide
