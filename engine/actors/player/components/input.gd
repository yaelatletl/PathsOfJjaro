...
# TO DO: get rid of redundant code and merge what's left into Player.gd; this being a single-player campaign, there is no value to the extra layer of abstraction: what matters is getting the single player behavior right! (if multiplayer DLC happens in future, the code can be refactored then but right now it only creates complexity)
