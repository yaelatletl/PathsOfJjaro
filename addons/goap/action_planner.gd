...
#
func get_plan(actor, goal: GoapGoal, blackboard = {}) -> Array:
	if not goal:
		return []
	print("Goal: %s" % goal.goap_get_class())
	#actor.actor_state.console_message("Goal: %s" % goal.goap_get_class())
	var desired_state = goal.get_desired_state().duplicate()

	if desired_state.is_empty():
		return []

	return _find_best_plan(actor, goal, desired_state, blackboard)
...
func _find_best_plan(actor, goal, desired_state, blackboard):
  ...
  # In case it doesn't find a valid path, it will return false.
	if _build_plans(actor, root, blackboard.duplicate()):
		var plans = _transform_tree_into_array(actor, root, blackboard)
		return _get_cheapest_plan(actor, plans)
...
#
func _get_cheapest_plan(actor, plans):
	var best_plan
	for p in plans:
		_print_plan(actor, p)
...
#
func _transform_tree_into_array(actor, p, blackboard):
	var plans = []
	...
	for c in p.children:
		for child_plan in _transform_tree_into_array(actor, c, blackboard):
			if p.action.has_method("get_cost"):
				child_plan.actions.push_back(p.action)
				child_plan.cost += p.action.get_cost(actor, blackboard)
...
#
func _print_plan(actor, plan):
	var actions = []
	for a in plan.actions:
		actions.push_back(a.goap_get_class())
	print({"cost": plan.cost, "actions": actions})
	actor.actor_state.console_message({"cost": plan.cost, "actions": actions})
