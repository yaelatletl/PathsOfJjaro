...
# TODO: decide when/how to draw automap later: redraws would probably be triggered by having Player's _physics_process emit a `player_did_move(...)` signal whenever the player turns or moves (i.e. whenever Player.global_transform changes); Radar could receive the same signal and redraw its center blip
...
# TODO: undecided on whether or not to support crosshair for easier aiming; excepting rocket launcher (which could have its own aim sight that appears when it is active), all weapons are centrally positioned in view (or equidistant for dual-wield) so targeting even distant bad guys and switches is relatively easy
@onready var crosshair := $CenterDisplay/Crosshair

# interaction board is for short-lived one-line messages telling player how to interact with a control panel # TODO: this should probably be centered on screen
...
func _ready():
	...
	# TODO: combine these signals into one? HUD would call WeaponManager.current_weapon.status to discover which transition it's in; an additional caveat is dual-wield weapons, where one gun is activating/deactivating while the other is active
...
func update_health_status(damage_type: Enums.DamageType = Enums.DamageType.NONE) -> void:
	...
	if damage_type != Enums.DamageType.NONE:
		pass # TODO: damage effect animations, e.g. red ColorRect "pain" pulse animation when struck by projectile; jagged blue-white jittery "shock" shader effect when hit by an energy bolt
...
func __inventory_status_changed(_item: InventoryManager.InventoryItem) -> void:
	redraw_current_weapon_status()
	# TODO: implement list of all available ammos down right side of screen

func __weapon_status_changed(_weapon: Weapon) -> void: # TODO: what should weapon signals pass as arguments, if anything? it may be best to pass nothing, leaving listeners to retrieve whatever they need from WeaponManager
	redraw_current_weapon_status()


func redraw_current_weapon_status() -> void:
	var weapon: Weapon = WeaponManager.current_weapon
	if not weapon: return # TODO: kludgy; WeaponManager.current_weapon should be set before HUD loads
...
func add_message(message, time_left, delete_on_signal = ""): # TODO: decide API and finish implementation
	var label = Label.new()
