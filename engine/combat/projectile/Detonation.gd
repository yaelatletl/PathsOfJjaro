extends StaticBody3D # TODO: what class should detonation effect (bullet, energy bolt, grenade, etc impact) use? AnimatedBody3D?
...
func _on_Area_body_entered(body) -> void: # TODO: only connect this if detonation_class.shrapnel_radius>0; if it does, set col.shape.radius and see if Godot automatically picks up all bodies within that radius and passes them here; if not, we'll need some other means to detect them
	
	# TODO: how best to determine if body is in same room as explosion? one option is to raycast from one to other, although we don't want that to fail if the ray impinges on a trivial scenery object (e.g. a bottle or chair) or partial wall (e.g. if player is 51% hidden from the explosion by a low wall which blocks a simple center-to-center raycast, they should still receive damage). Q. Is there a way to raycast a 2D area instead of a point?
	...
	# TODO: the Explosion instance should pass itself to every body that is passed here, and let the body itself decide how it responds to it
