...
uniform sampler2D baseColor : hint_albedo;
uniform sampler2D normalMap : hint_normal;
uniform sampler2D ORM : hint_black_albedo;
...
uniform float g_DetailTileRate;
float GetTileRate()
{
...
void fragment(){
	vec3 wrld_vertex = (CAMERA_MATRIX * vec4(VERTEX, 1.0)).rgb;
	vec3 sp = GetTileRate() * wrld_vertex;
	...
	vec4 normal;
	vec4 orm;
	hex2colTex(albedo, weights, baseColor, st0, 1.0, 0.5);
	hex2colTex(normal, weights, normalMap, st0, 1.0, 0.5);
	hex2colTex(orm, weights, ORM, st0, 1.0, 0.5);
	AO = orm.r;
	ROUGHNESS = orm.g;
	METALLIC = orm.b;
	ALBEDO = albedo.rgb;
