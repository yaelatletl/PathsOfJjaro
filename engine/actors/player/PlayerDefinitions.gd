...
# TODO: Boomer (Pfhor ship) maps have a low gravity flag, but I assume we can get the same effect by loading a different physics model for those levels? (really don't want lots of flag modifiers)



# TODO: in 3D Physics layers, rename "Level" to Map/MapGeometry/Wall/Surface/Solid/Exterior/Architecture/Shell or something else that's descriptive as "Level" is unhelpful (in mapping terms, a Level is the map geometry *and* all of the objects inside it)

# TODO: Player currently uses CapsuleShape to detect floor; at what point does capsule slide off a ledge? (not sure what M2's collision cylinder does - it might remain on ledge as long as any part of its base remains in contact; capsule is useful as it can push player out from wall so the model doesn't appear to slide down wall partly embedded in it; we might want to add a smaller cylinder to bottom of capsule to widen it a bit; also need to decide on player's Projectile/Explosion hit box, which may be a slightly narrower capsule or cylinder; there is also Player-NPC collisions to consider, which may need a wider collision cylinder to prevent NPC models' extremities appearing embedded in Player when both bodies are very close/touching)


# TODO: these were copied from M3 physics so need converted to Godot quantities; most properties have yet to be connected to implementation
...
const CRAWL_PHYSICS := {
	"maximum_forward_velocity": 0.5 * INPUT_AXIS_MULTIPLIER, # 0.07142639, # TODO: for now, multiply all velocity-related fields by 14 to get values relative to forward walk speed (we can figure out the final multiplier to emulate M2 speeds later)
	...
	"max_step_height": 0.5, # TODO: what should this be when crawling?
	"max_jump_height": 0.0, # auto-jump is only available when sprinting
	
	"gravity":         10.0, # Gravity force #45 is okay, don't change it 
	"air_friction":    50.0, # TODO: think this is airborne_deceleration
...
const WALK_PHYSICS := {
	"maximum_forward_velocity": 1.0 * INPUT_AXIS_MULTIPLIER, # 0.07142639, # TODO: for now, multiply all velocity-related fields by 14 to get values relative to forward walk speed (we can figure out the final multiplier to emulate M2 speeds later)
	...
	# TODO: these aren't currently hooked up
	...
	"angular_recentering_velocity": 0.75, # TODO: how quickly the camera vertically auto-recenters when moving forward/backward (note: moving sideways does not auto-recenter)
	
	
	#"radius": 0.25,
	#"height": 0.7999878,
	#"camera_height": 0.19999695,
	"splash_height": 0.5, # TODO: I assume this puts waterline around waist when swimming on surface of liquid
...
const SPRINT_PHYSICS := {
	"maximum_forward_velocity": 1.75 * INPUT_AXIS_MULTIPLIER * 1.3, # 0.125, # TODO: check Player.linear_velocity is increased by 175% when sprinting vs walking (currently sprinting feels a bit slow)
