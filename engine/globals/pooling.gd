...
# TO DO: this is a premature optimization! (allocating and freeing projectile instances is unlikely to be a critical bottleneck and there's no point guessing until the engine is functionally complete and can be performance-profiled); for now, instantiate and free single-use Projectile, Explosion, etc objects to keep the code simple, and get rid of this file
...
#	1: preload("res://engine/projectiles/PlasmaBolt.tscn"),
#	2: preload("res://engine/projectiles/Grenade.tscn"),
}
...
#	print("Position passed to add_projectile: " + position)
	...
	if found == null:
		...
		projectile_instance.request_destroy.connect(_on_projectile_request_destroy.bind(projectile_instance))
	else:
...
#Creates a dummy actor to be used for portal physics # TO DO: delete this (supporting 5D space is a waste of time; it's either accidental, e.g. Arrival, or a secret gimmick, Ingue Ferroque)
