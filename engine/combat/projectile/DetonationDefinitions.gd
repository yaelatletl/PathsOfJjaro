...
const DETONATION_DEFINITIONS := [ # originally Physics Effects, but we've separated visual-only effects from detonations
	...
	{
		"damage_type": Enums.DamageType.PROJECTILE, # TODO: might want to put damage_type to detonation_type mappings in their own table; alternatively, we might use damage_type + projectile_type or some other combination 
	...
	
	{
		...
		"name": "grenade explosion", # TODO: naming conversion for mapping human-readable names to scene names/panes (simplest is just to insert the definition type and definition name strings into "res://assets/%s/%s.tscn" after validating strings, e.g. "res://assets/detonations/grenade explosion.tscn", although we could convert names to e.g. "GrenadeExplosion.tscn" if TitleCase works better)
		...
		#	"media_effect": false, # TODO: true for liquid splash effects only; delete here
		#},
		...
		#"alien_damage": false, # TODO: shouldn't need this
		"base": 80,
		"random": 20,
		"scale": 0.0,
