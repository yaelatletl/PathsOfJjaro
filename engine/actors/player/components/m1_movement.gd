...
# TO DO: how does this differ from movement.gd? where there is M1 vs M2 discrepancy, let's standardize on M2 behaviors as those are the more polished
...
func _crouch(input : Dictionary, _delta :float) -> void:
	...
	if not actor.head.is_colliding():
		...
		shape = lerp(shape, 1.7 - (get_key(input, "crouch") * 1.5), w_speed  * _delta)
