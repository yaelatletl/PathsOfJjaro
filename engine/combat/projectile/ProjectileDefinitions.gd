...
const PROJECTILE_DEFINITIONS := [
	...
	
	{
		...
		#"ticks_between_contrails": DetonationType.ROCKET_EXPLOSION, # TODO: exporter bug; should be int
		#"maximum_contrails": DetonationType.ROCKET_EXPLOSION, # TODO: ditto
		
		
		"media_projectile_promotion": null, # minor/major fusion, rocket; i.e. these penetrate liquids; all others do not # note: minor/major fusion when it hits water promotes to minor/major fusion dispersal which immediately explodes (-ve range, originally possibly -1 or other flag-like int without fixed-point conversion); TODO: transitions can also handle switch to Detonation/Effect
		...
		#	"scale": 0.0,
		#	"flags": {
		#		"alien_damage": false, # TODO: get rid of this; if we need an alien_damage flag at all it should go on Detonation or Damage; however, define separate ProjectileType.HUMAN_PROJECTILE and ProjectileType.ALIEN_PROJECTILE first
		#	},
		#},
		"flags": {
			...
			"alien": false, # TODO: what does this do in Classic?
			
			# pathfinding
			"guided": false,
			
			"affected_by_gravity": false, # TODO: combine these three into dict where keys are all gravities used in game and values are gravity to apply to projectile (0.0 = none, 1.0 = 1G)
			...
			"melee": false, # TODO: what does this do in Classic? can we get rid of it? need to check AO code
			
			
			# TODO: these flags are true for ball only; any special cases like this should be defined as Projectile subclasses that implement their own gameplay behavior
			#"persistent_and_virulent": false,
			#"becomes_item_on_detonation": false,
			
			# TODO: rest of these flags should be replaced with boundary transitions
